...
n_classes = 10
batch_sizes = [50, 100, 150, 200] #100 input
accu = []
...
def train_neural_network(x, batch_size):
	prediction = neural_network_model(x)
	cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(prediction,y))
	optimizer = tf.train.AdamOptimizer().minimize(cost)

	hm_epochs = 5 #feedforward  
...
plt.plot(batch_sizes, accu)
plt.axis([batch_sizes[0]-5,batch_sizes[-1]+5, accu[0]-0.1,accu[-1]+0.1])
plt.show()
plt.pause(0)
